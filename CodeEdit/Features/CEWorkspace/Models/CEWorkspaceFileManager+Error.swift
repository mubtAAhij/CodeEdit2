...
extension CEWorkspaceFileManager {
    ...
    enum FileManagerError: LocalizedError {
        ...
        var errorDescription: String? {
            ...
            case .fileNotIndexed:
                return String(localized: "file-manager.error.file-not-indexed", defaultValue: "File not found in CodeEdit", comment: "Error when file is not indexed in workspace")
            case .originFileNotFound:
        ...
        var recoverySuggestion: String? {
            ...
            case .destinationFileExists:
                return String(localized: "file-manager.recovery.rename-or-remove", defaultValue: "Use a different file name or remove the conflicting file.", comment: "Recovery suggestion for existing destination")
            case .invalidFileName:
                return String(localized: "file-manager.recovery.filename-requirements", defaultValue: "File names must not contain the : character and be less than 256 characters.", comment: "Recovery suggestion for invalid file name")
            }
        }
    }
}
