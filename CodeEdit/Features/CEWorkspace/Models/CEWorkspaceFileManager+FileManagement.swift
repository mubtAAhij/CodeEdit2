...
extension CEWorkspaceFileManager {
    ...
    public func delete(file: CEWorkspaceFile, confirmDelete: Bool = true) throws {
        ...
        deleteConfirmation.messageText = String(
            format: String(
                ...
                comment: "Confirmation message when deleting a single file"
            ),
            fileName
        )
        deleteConfirmation.informativeText = String(
            localized: "file-management.delete.warning-message",
            defaultValue: "This item will be deleted immediately. You can't undo this action.",
            comment: "Warning text about permanent deletion"
        )
        deleteConfirmation.alertStyle = .critical
        deleteConfirmation.addButton(
            withTitle: String(
                localized: "file-management.delete.delete-button",
                defaultValue: "Delete",
        ...
        deleteConfirmation.addButton(
            withTitle: String(
                localized: "file-management.delete.cancel-button",
                defaultValue: "Cancel",
                comment: "Button to cancel deletion"
            )
        )
    ...
    public func batchDelete(files: Set<CEWorkspaceFile>, confirmDelete: Bool = true) throws {
        ...
        deleteConfirmation.informativeText = String(
            format: String(
                localized: "file-management.batch-delete.warning-message",
                defaultValue: "%d items will be deleted immediately. You cannot undo this action.",
                comment: "Warning text about permanent batch deletion"
            ),
            files.count
        )
        deleteConfirmation.alertStyle = .critical
        deleteConfirmation.addButton(
            withTitle: String(
                localized: "file-management.batch-delete.delete-button",
                defaultValue: "Delete",
                comment: "Button to confirm batch deletion"
            )
        )
        deleteConfirmation.buttons.last?.hasDestructiveAction = true
        deleteConfirmation.addButton(
            withTitle: String(
                localized: "file-management.batch-delete.cancel-button",
                defaultValue: "Cancel",
                comment: "Button to cancel batch deletion"
            )
        )
    ...
    public func duplicate(file: CEWorkspaceFile) throws {
        ...
        while fileManager.fileExists(atPath: fileUrl.path) {
            ...
            let duplicateName = String(
                format: String(
                    localized: "file-management.duplicate.copy-suffix",
                    defaultValue: "%@ copy%@",
                    comment: "Format for duplicate file names: original name + 'copy' + extension"
                ),
                fileName,
                fileExtension
            )
