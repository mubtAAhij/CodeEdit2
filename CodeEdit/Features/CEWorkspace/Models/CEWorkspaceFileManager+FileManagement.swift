...
extension CEWorkspaceFileManager {
    ...
    public func delete(file: CEWorkspaceFile, confirmDelete: Bool = true) throws {
        ...
        deleteConfirmation.messageText = String(format: String(localized: "file-manager.delete.confirm-message", defaultValue: "Do you want to delete \"%@\"?", comment: "Confirmation message for deleting a file"), fileName)
        deleteConfirmation.informativeText = String(localized: "file-manager.delete.confirm-info", defaultValue: "This item will be deleted immediately. You can't undo this action.", comment: "Information text for delete confirmation")
        deleteConfirmation.alertStyle = .critical
        deleteConfirmation.addButton(withTitle: String(localized: "file-manager.delete.delete-button", defaultValue: "Delete", comment: "Button to confirm deletion"))
        deleteConfirmation.buttons.last?.hasDestructiveAction = true
        deleteConfirmation.addButton(withTitle: String(localized: "file-manager.delete.cancel-button", defaultValue: "Cancel", comment: "Button to cancel deletion"))
    ...
    public func batchDelete(files: Set<CEWorkspaceFile>, confirmDelete: Bool = true) throws {
        let deleteConfirmation = NSAlert()
        deleteConfirmation.messageText = String(format: String(localized: "file-manager.batch-delete.confirm-message", defaultValue: "Are you sure you want to delete the %d selected items?", comment: "Confirmation message for batch deleting files"), files.count)
        // swiftlint:disable:next line_length
        deleteConfirmation.informativeText = String(format: String(localized: "file-manager.batch-delete.confirm-info", defaultValue: "%d items will be deleted immediately. You cannot undo this action.", comment: "Information text for batch delete confirmation"), files.count)
        deleteConfirmation.alertStyle = .critical
        deleteConfirmation.addButton(withTitle: String(localized: "file-manager.batch-delete.delete-button", defaultValue: "Delete", comment: "Button to confirm batch deletion"))
        deleteConfirmation.buttons.last?.hasDestructiveAction = true
        deleteConfirmation.addButton(withTitle: String(localized: "file-manager.batch-delete.cancel-button", defaultValue: "Cancel", comment: "Button to cancel batch deletion"))
    ...
    public func duplicate(file: CEWorkspaceFile) throws {
        ...
        while fileManager.fileExists(atPath: fileUrl.path) {
            ...
            fileUrl = fileUrl.deletingLastPathComponent().appending(path: String(format: String(localized: "file-manager.duplicate.name-format", defaultValue: "%@ copy%@", comment: "Format for duplicate file name: original name, then 'copy', then extension"), fileName, fileExtension))
        }
