...
extension CEWorkspaceFileManager {
    ...
    public func delete(file: CEWorkspaceFile, confirmDelete: Bool = true) throws {
        ...
        deleteConfirmation.messageText = String(format: String(localized: "file_management.delete.confirmation_message", defaultValue: "Do you want to delete \"%@\"?", comment: "Confirmation message for deleting a single file"), fileName)
        deleteConfirmation.informativeText = String(localized: "file_management.delete.warning_message", defaultValue: "This item will be deleted immediately. You can't undo this action.", comment: "Warning that deletion is permanent")
        deleteConfirmation.alertStyle = .critical
        deleteConfirmation.addButton(withTitle: String(localized: "file_management.delete.button", defaultValue: "Delete", comment: "Delete button title"))
        deleteConfirmation.buttons.last?.hasDestructiveAction = true
        deleteConfirmation.addButton(withTitle: String(localized: "file_management.cancel.button", defaultValue: "Cancel", comment: "Cancel button title"))
    ...
    public func batchDelete(files: Set<CEWorkspaceFile>, confirmDelete: Bool = true) throws {
        let deleteConfirmation = NSAlert()
        deleteConfirmation.messageText = String(format: String(localized: "file_management.batch_delete.confirmation_message", defaultValue: "Are you sure you want to delete the %d selected items?", comment: "Confirmation message for batch deleting files"), files.count)
        // swiftlint:disable:next line_length
        deleteConfirmation.informativeText = String(format: String(localized: "file_management.batch_delete.warning_message", defaultValue: "%d items will be deleted immediately. You cannot undo this action.", comment: "Warning that batch deletion is permanent"), files.count)
        deleteConfirmation.alertStyle = .critical
        deleteConfirmation.addButton(withTitle: String(localized: "file_management.batch_delete.button", defaultValue: "Delete", comment: "Delete button title for batch deletion"))
        deleteConfirmation.buttons.last?.hasDestructiveAction = true
        deleteConfirmation.addButton(withTitle: String(localized: "file_management.batch_cancel.button", defaultValue: "Cancel", comment: "Cancel button title for batch deletion"))
    ...
    public func duplicate(file: CEWorkspaceFile) throws {
        ...
        while fileManager.fileExists(atPath: fileUrl.path) {
            ...
            fileUrl = fileUrl.deletingLastPathComponent().appending(path: String(format: String(localized: "file_management.duplicate.copy_suffix", defaultValue: "%@ copy%@", comment: "Format for duplicated file name with copy suffix"), fileName, fileExtension))
        }
