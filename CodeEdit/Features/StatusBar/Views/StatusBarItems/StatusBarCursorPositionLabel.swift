    ...
    struct LineLabel: View {
        ...
        func getLabel() -> String {
            ...
            if cursorPositions.count > 1 {
                return String(format: String(localized: "status-bar.cursor.selected-ranges", defaultValue: "%d selected ranges", comment: "Number of selected text ranges"), cursorPositions.count)
            }
            ...
            if cursorPositions[0].range.length > 0 {
                ...
                if modifierKeys.contains(.option) {
                    return String(format: String(localized: "status-bar.cursor.char-range", defaultValue: "Char: %d Len: %d", comment: "Character position and length"), cursorPositions[0].range.location, cursorPositions[0].range.length)
                }

                let lineCount = getLines(cursorPositions[0].range)

                if lineCount > 1 {
                    return String(format: String(localized: "status-bar.cursor.line-count", defaultValue: "%d lines", comment: "Number of selected lines"), lineCount)
                }

                return String(format: String(localized: "status-bar.cursor.char-count", defaultValue: "%d characters", comment: "Number of selected characters"), cursorPositions[0].range.length)
            }

            // When the option key is pressed display the character offset.
            if modifierKeys.contains(.option) {
                return String(format: String(localized: "status-bar.cursor.char-offset", defaultValue: "Char: %d Len: 0", comment: "Character offset with no selection"), cursorPositions[0].range.location)
            }

            // When there's a single cursor, display the line and column.
            return String(format: String(localized: "status-bar.cursor.line-col", defaultValue: "Line: %d  Col: %d", comment: "Cursor line and column position"), cursorPositions[0].start.line, cursorPositions[0].start.column)
        }
    }
}
