    ...
    struct LineLabel: View {
        ...
        func getLabel() -> String {
            ...
            if cursorPositions.count > 1 {
                return String(format: String(localized: "status-bar.cursor.selected-ranges", defaultValue: "%d selected ranges", comment: "Number of selected ranges in status bar"), cursorPositions.count)
            }
            ...
            if cursorPositions[0].range.length > 0 {
                ...
                if modifierKeys.contains(.option) {
                    return String(format: String(localized: "status-bar.cursor.char-len", defaultValue: "Char: %d Len: %d", comment: "Character position and length in status bar"), cursorPositions[0].range.location, cursorPositions[0].range.length)
                }

                let lineCount = getLines(cursorPositions[0].range)

                if lineCount > 1 {
                    return String(format: String(localized: "status-bar.cursor.lines-count", defaultValue: "%d lines", comment: "Number of selected lines in status bar"), lineCount)
                }

                return String(format: String(localized: "status-bar.cursor.characters-count", defaultValue: "%d characters", comment: "Number of selected characters in status bar"), cursorPositions[0].range.length)
            }

            // When the option key is pressed display the character offset.
            if modifierKeys.contains(.option) {
                return String(format: String(localized: "status-bar.cursor.char-offset", defaultValue: "Char: %d Len: 0", comment: "Character offset in status bar"), cursorPositions[0].range.location)
            }

            // When there's a single cursor, display the line and column.
            return String(format: String(localized: "status-bar.cursor.line-col", defaultValue: "Line: %d  Col: %d", comment: "Line and column position in status bar"), cursorPositions[0].start.line, cursorPositions[0].start.column)
        }
    }
}
