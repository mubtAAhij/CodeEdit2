    ...
    struct LineLabel: View {
        ...
        func getLabel() -> String {
            ...
            if cursorPositions.count > 1 {
                return String(format: String(localized: "status-bar.cursor.selected-ranges", defaultValue: "%d selected ranges", comment: "Number of selected ranges in editor"), cursorPositions.count)
            }
            ...
            if cursorPositions[0].range.length > 0 {
                ...
                if modifierKeys.contains(.option) {
                    return String(format: String(localized: "status-bar.cursor.char-len", defaultValue: "Char: %d Len: %d", comment: "Character position and length"), cursorPositions[0].range.location, cursorPositions[0].range.length)
                }
            ...
            return String(format: String(localized: "status-bar.cursor.line-col", defaultValue: "Line: %d  Col: %d", comment: "Line and column position"), cursorPositions[0].start.line, cursorPositions[0].start.column)
        }
    }
}
