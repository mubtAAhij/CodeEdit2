        ...
        func getLabel() -> String {
            ...
            if cursorPositions[0].range.length > 0 {
                ...
                if modifierKeys.contains(.option) {
                    return String(format: String(localized: "status-bar.cursor.char-location-length", defaultValue: "Char: %d Len: %d", comment: "Character location and length"), cursorPositions[0].range.location, cursorPositions[0].range.length)
                }

                let lineCount = getLines(cursorPositions[0].range)

                if lineCount > 1 {
                    return String(format: String(localized: "status-bar.cursor.lines-count", defaultValue: "%d lines", comment: "Number of lines selected"), lineCount)
                }

                return String(format: String(localized: "status-bar.cursor.characters-count", defaultValue: "%d characters", comment: "Number of characters selected"), cursorPositions[0].range.length)
            }

            // When the option key is pressed display the character offset.
            if modifierKeys.contains(.option) {
                return String(format: String(localized: "status-bar.cursor.char-location-zero", defaultValue: "Char: %d Len: 0", comment: "Character location with zero length"), cursorPositions[0].range.location)
            }

            // When there's a single cursor, display the line and column.
            return String(format: String(localized: "status-bar.cursor.line-column", defaultValue: "Line: %d  Col: %d", comment: "Line and column position"), cursorPositions[0].start.line, cursorPositions[0].start.column)
        }
    }
}
