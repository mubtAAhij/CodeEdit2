        ...
        func getLabel() -> String {
            ...
            if cursorPositions[0].range.length > 0 {
                ...
                if modifierKeys.contains(.option) {
                    return String(format: String(localized: "status-bar.cursor.char-range", defaultValue: "Char: %d Len: %d", comment: "Character position and length of selection"), cursorPositions[0].range.location, cursorPositions[0].range.length)
                }
            ...
            if modifierKeys.contains(.option) {
                return String(format: String(localized: "status-bar.cursor.char-position", defaultValue: "Char: %d Len: 0", comment: "Character position with no selection"), cursorPositions[0].range.location)
            }

            // When there's a single cursor, display the line and column.
            return String(format: String(localized: "status-bar.cursor.line-col", defaultValue: "Line: %d  Col: %d", comment: "Line and column position of cursor"), cursorPositions[0].start.line, cursorPositions[0].start.column)
        }
    }
}
