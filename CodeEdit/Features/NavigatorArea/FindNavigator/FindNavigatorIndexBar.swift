            ...
            if shouldShow {
                HStack(alignment: .center) {
                    ProgressView(value: progress, total: 1.0) {
                        ...
                        HStack {
                            Text(String(format: String(localized: "find-navigator.indexing-progress", defaultValue: "Indexing %d%%", comment: "Progress message shown while indexing workspace files"), Int(progress * 100)))
                                .font(.system(size: 10))
                                .animation(.none)
                        }
                    }
