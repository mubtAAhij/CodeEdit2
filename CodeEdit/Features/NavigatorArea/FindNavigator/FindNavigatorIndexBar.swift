            ...
            if shouldShow {
                HStack(alignment: .center) {
                    ProgressView(value: progress, total: 1.0) {
                        ...
                        HStack {
                            Text(String(format: String(localized: "find-navigator.indexing-progress", defaultValue: "Indexing %d%%", comment: "Progress percentage for workspace file indexing"), Int(progress * 100)))
                                .font(.system(size: 10))
                                .animation(.none)
                        }
                    }
                    // swiftlint:disable:next line_length
                    .help(String(localized: "find-navigator.indexing-help", defaultValue: "Indexing current workspace files for search. Searches performed while indexing may return incomplete results.", comment: "Help text explaining the indexing process and its limitations"))
                }
