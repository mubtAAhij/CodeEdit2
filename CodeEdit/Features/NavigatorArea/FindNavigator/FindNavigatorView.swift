        ...
        VStack {
            ...
            if findNavigatorStatus == .found {
                HStack(alignment: .center) {
                    Text(String(format: String(localized: "find_navigator.results_summary", defaultValue: "%#@results@ in %#@files@", comment: "Search results summary with counts"), searchResultCount, foundFilesCount))
                        .font(.system(size: 10))
                }
            ...
            case .replaced(let updatedFiles):
                CEContentUnavailableView(
                    ...
                    description: String(format: String(localized: "find_navigator.replaced_success_message", defaultValue: "Successfully replaced terms across %#@files@", comment: "Message showing successful replacement across files"), updatedFiles),
