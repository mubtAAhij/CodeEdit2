        ...
        VStack {
            ...
            if findNavigatorStatus == .found {
                HStack(alignment: .center) {
                    Text(String(format: String(localized: "find_navigator.results_summary", defaultValue: "%#@results@ in %#@files@", comment: "Summary showing number of search results and files"), searchResultCount, foundFilesCount))
                        .font(.system(size: 10))
                }
            ...
            case .found:
                if self.searchResultCount == 0 {
                    CEContentUnavailableView(
                        ...
                        description: String(format: String(localized: "find_navigator.no_results_for_query", defaultValue: "No Results for \"%@\" in Project", comment: "Message when no results found for specific query"), state.searchQuery),
            ...
            case .replaced(let updatedFiles):
                CEContentUnavailableView(
                    ...
                    description: String(format: String(localized: "find_navigator.replaced_success_message", defaultValue: "Successfully replaced terms across %#@files@", comment: "Success message showing how many files were updated"), updatedFiles),
                    systemImage: "checkmark.circle.fill"
                )
            case .failed(let errorMessage):
                CEContentUnavailableView(
                    String(localized: "find_navigator.error_occurred", defaultValue: "An Error Occurred", comment: "Title shown when an error occurs during find/replace"),
