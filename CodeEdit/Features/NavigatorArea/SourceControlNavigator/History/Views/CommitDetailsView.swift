    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            HStack {
                ...
                Text(String(localized: "source-control.commit-details.title", defaultValue: "Commit Details", comment: "Title for commit details view"))
            ...
            if let commit = commit {
                ...
                if !commitChanges.isEmpty {
                    ...
                    CEContentUnavailableView(String(localized: "source-control.commit-details.no-changes", defaultValue: "No Changes", comment: "Empty state message when a commit has no changes"))
                }
            } else {
                Spacer()
            }
