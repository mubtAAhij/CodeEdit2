        ...
        if let currentBranch = sourceControlManager.currentBranch {
            HStack {
                if currentBranch.upstream == nil {
                    Label(title: {
                        Text(String(format: String(localized: "source-control.sync.no-tracked-branch", defaultValue: "No tracked branch for '%@'", comment: "Message when there is no tracked branch for the current branch"), sourceControlManager.currentBranch?.name ?? ""))
                    }, icon: {
                        Image(symbol: "branch")
                ...
                if sourceControlManager.numberOfUnsyncedCommits.behind > 0 {
                    Button {
                        ...
                        Text(String(localized: "source-control.sync.pull-button", defaultValue: "Pull...", comment: "Button to pull changes from remote"))
                    }
                    .disabled(isLoading)
                    ...
                    Button {
                        ...
                        Text(String(localized: "source-control.sync.push-button", defaultValue: "Push...", comment: "Button to push changes to remote"))
                    }
                    .disabled(isLoading)
                }
            }
    ...
    func pull() {
        Task(priority: .background) {
            ...
            do {
                ...
                await sourceControlManager.showAlertForError(title: String(localized: "source-control.sync.pull-error-title", defaultValue: "Failed to pull", comment: "Error title when pull operation fails"), error: error)
            }
    ...
    func push() {
        Task(priority: .background) {
            ...
            do {
                ...
                await sourceControlManager.showAlertForError(title: String(localized: "source-control.sync.push-error-title", defaultValue: "Failed to push", comment: "Error title when push operation fails"), error: error)
            }
            self.isLoading = false
        }
    }
