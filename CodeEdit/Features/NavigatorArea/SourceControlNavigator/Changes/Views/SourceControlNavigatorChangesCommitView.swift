            ...
            VStack(spacing: 0) {
                ...
                .safeAreaInset(edge: .bottom, spacing: 0) {
                    if showDetails {
                        VStack {
                            TextField(
                                String(localized: "source-control.detailed-description-placeholder", defaultValue: "Detailed description", comment: "Placeholder for detailed commit description field"),
                                text: $details,
                                axis: .vertical
                            )
                ...
                VStack(spacing: 0) {
                    if showDetails {
                        Toggle(isOn: $ammend) {
                            Text(String(localized: "source-control.amend-commit", defaultValue: "Amend", comment: "Toggle label to amend previous commit"))
                ...
                HStack(spacing: 8) {
                    ...
                    Menu(isCommiting ? String(localized: "source-control.committing", defaultValue: "Committing...", comment: "Menu label while committing") : String(localized: "source-control.commit", defaultValue: "Commit", comment: "Menu label to commit changes")) {
                            ...
                            Task {
                                ...
                                do {
                                    ...
                                    await sourceControlManager.showAlertForError(
                                        title: String(localized: "source-control.failed-to-commit", defaultValue: "Failed to commit", comment: "Error title when commit operation fails"),
                                        error: error
                                    )
                                }
                                do {
                                    ...
                                    await sourceControlManager.showAlertForError(title: String(localized: "source-control.failed-to-push", defaultValue: "Failed to push", comment: "Error title when push operation fails"), error: error)
                                }
                        ...
                        Task {
                            ...
                            do {
                                ...
                                await sourceControlManager.showAlertForError(title: String(localized: "source-control.failed-to-commit", defaultValue: "Failed to commit", comment: "Error title when commit operation fails"), error: error)
                            }
                            self.isCommiting = false
                        }
                    }
