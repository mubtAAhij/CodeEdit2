                ...
                if let behind = item.branch?.behind, behind > 0 {
                    HStack(spacing: 0) {
                        Image(systemName: "arrow.down")
                            .imageScale(.small)
                        Text(String(format: String(localized: "source-control.repository-item.behind-count", defaultValue: "%d", comment: "Number of commits the local branch is behind the remote"), behind))
                            .font(.system(size: 11))
                    }
                }
                if let ahead = item.branch?.ahead, ahead > 0 {
                    HStack(spacing: 0) {
                        Image(systemName: "arrow.up")
                            .imageScale(.small)
                        Text(String(format: String(localized: "source-control.repository-item.ahead-count", defaultValue: "%d", comment: "Number of commits the local branch is ahead of the remote"), ahead))
                            .font(.system(size: 11))
                    }
                }
            }
