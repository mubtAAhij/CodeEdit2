...
extension ProjectNavigatorMenu {
    ...
    func newFile() {
        ...
        do {
            if let newFile = try workspace?.workspaceFileManager?.addFile(fileName: String(localized: "project-navigator.untitled-file", defaultValue: "untitled", comment: "Default name for new untitled file"), toFile: item) {
                workspace?.listenerModel.highlightedFileItem = newFile
                workspace?.editorManager?.openTab(item: newFile)
            }
        } catch {
            let alert = NSAlert(error: error)
            alert.addButton(withTitle: String(localized: "project-navigator.dismiss", defaultValue: "Dismiss", comment: "Dismiss button in error alert"))
            alert.runModal()
        }
    }
    ...
    func newFileFromClipboard() {
        ...
        do {
            ...
            if let clipBoardContent, !clipBoardContent.isEmpty, let newFile = try workspace?
                ...
                .addFile(
                    fileName: String(localized: "project-navigator.untitled-file", defaultValue: "untitled", comment: "Default name for new untitled file"),
                    toFile: item,
    ...
    func newFolder() {
        guard let item else { return }
        do {
            if let newFolder = try workspace?.workspaceFileManager?.addFolder(folderName: String(localized: "project-navigator.untitled-folder", defaultValue: "untitled", comment: "Default name for new untitled folder"), toFile: item) {
    ...
    func newFolderFromSelection() {
        ...
        /// Get 'New Folder' name.
        var newFolderURL = parent.url.appendingPathComponent(String(localized: "project-navigator.new-folder-with-items", defaultValue: "New Folder With Items", comment: "Default name for new folder containing selected items"), conformingTo: .folder)
        var folderNumber = 0
        while workspaceFileManager.fileManager.fileExists(atPath: newFolderURL.path) {
            folderNumber += 1
            newFolderURL = parent.url.appending(path: String(format: String(localized: "project-navigator.new-folder-with-items-numbered", defaultValue: "New Folder With Items %d", comment: "Numbered default name for new folder containing selected items"), folderNumber))
        }
