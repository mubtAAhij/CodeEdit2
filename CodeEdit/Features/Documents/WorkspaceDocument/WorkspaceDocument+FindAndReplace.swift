...
extension WorkspaceDocument.SearchState {
    ...
    func findAndReplace(query: String, replacingTerm: String) async throws {
        ...
        if updatedFilesCount == 0 && errorCount == 0 {
            ...
            await setStatus(.failed(errorMessage: String(format: String(localized: "find-replace.no-files-matched", defaultValue: "No files in the workspace matched: %@", comment: "No files matched search query"), query)))
        } else if updatedFilesCount == 0 && errorCount > 0 {
            // All files failed to updated
            await setStatus(
                .failed(
                    errorMessage: String(format: String(localized: "find-replace.all-files-failed", defaultValue: "All files failed to update. (%d) errors occurred. Check logs for more information", comment: "All files failed to update"), errorCount)
                )
            )
        } else if updatedFilesCount > 0 && errorCount > 0 {
    ...
    func replaceRange(
        ...
        guard let fileContent = try? String(contentsOf: file, encoding: .utf8) else {
            let alert = NSAlert()
            alert.messageText = String(localized: "find-replace.error-title", defaultValue: "Error", comment: "Error title")
            alert.informativeText = String(format: String(localized: "find-replace.read-error", defaultValue: "An error occurred while reading file contents of: %@", comment: "File read error"), file.absoluteString)
            alert.alertStyle = .critical
            alert.addButton(withTitle: String(localized: "find-replace.ok-button", defaultValue: "OK", comment: "OK button"))
            alert.runModal()
        ...
        do {
            ...
            let alert = NSAlert()
            alert.messageText = String(localized: "find-replace.error-title", defaultValue: "Error", comment: "Error title")
            alert.informativeText = String(format: String(localized: "find-replace.write-error", defaultValue: "An error occurred while writing to: %@", comment: "File write error"), error.localizedDescription)
            alert.alertStyle = .critical
            alert.addButton(withTitle: String(localized: "find-replace.ok-button", defaultValue: "OK", comment: "OK button"))
            alert.runModal()
        }
