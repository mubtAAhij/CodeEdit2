...
extension WorkspaceDocument.SearchState {
    ...
    func findAndReplace(query: String, replacingTerm: String) async throws {
        ...
        if updatedFilesCount == 0 && errorCount == 0 {
            ...
            await setStatus(.failed(errorMessage: String(format: String(localized: "find-and-replace.no-files-matched", defaultValue: "No files in the workspace matched: %@", comment: "Error message when no files match search query"), query)))
        } else if updatedFilesCount == 0 && errorCount > 0 {
            // All files failed to updated
            await setStatus(
                .failed(
                    errorMessage: String(format: String(localized: "find-and-replace.all-files-failed", defaultValue: "All files failed to update. (%d) errors occurred. Check logs for more information", comment: "Error message when all files fail to update"), errorCount)
                )
            )
        } else if updatedFilesCount > 0 && errorCount > 0 {
            ...
            await setStatus(
                .failed(
                    errorMessage: String(format: String(localized: "find-and-replace.partial-success", defaultValue: "%d successfully updated, %d errors occurred. Please check logs for more information.", comment: "Message when some files updated successfully and some failed"), updatedFilesCount, errorCount)
                )
            )
        } else {
    ...
    func replaceRange(
        ...
        guard let fileContent = try? String(contentsOf: file, encoding: .utf8) else {
            let alert = NSAlert()
            alert.messageText = String(localized: "find-and-replace.error-title", defaultValue: "Error", comment: "Alert title for file read error")
            alert.informativeText = String(format: String(localized: "find-and-replace.read-error", defaultValue: "An error occurred while reading file contents of: %@", comment: "Error message when reading file fails"), file.absoluteString)
            alert.alertStyle = .critical
            alert.addButton(withTitle: String(localized: "find-and-replace.ok-button", defaultValue: "OK", comment: "OK button in error alert"))
            alert.runModal()
        ...
        do {
            ...
            let alert = NSAlert()
            alert.messageText = String(localized: "find-and-replace.write-error-title", defaultValue: "Error", comment: "Alert title for file write error")
            alert.informativeText = String(format: String(localized: "find-and-replace.write-error", defaultValue: "An error occurred while writing to: %@", comment: "Error message when writing file fails"), error.localizedDescription)
            alert.alertStyle = .critical
            alert.addButton(withTitle: String(localized: "find-and-replace.write-ok-button", defaultValue: "OK", comment: "OK button in write error alert"))
            alert.runModal()
        }
    }
