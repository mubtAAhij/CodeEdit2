...
extension WorkspaceDocument.SearchState {
    ...
    func findAndReplace(query: String, replacingTerm: String) async throws {
        ...
        if updatedFilesCount == 0 && errorCount == 0 {
            ...
            await setStatus(.failed(errorMessage: String(format: String(localized: "find-and-replace.no-files-matched", defaultValue: "No files in the workspace matched: %@", comment: "Error when no files match the search query"), query)))
        } else if updatedFilesCount == 0 && errorCount > 0 {
            // All files failed to updated
            await setStatus(
                .failed(
                    errorMessage: String(format: String(localized: "find-and-replace.all-files-failed", defaultValue: "All files failed to update. (%d) errors occurred. Check logs for more information", comment: "Error when all files failed to update"), errorCount)
                )
            )
        } else if updatedFilesCount > 0 && errorCount > 0 {
            ...
            await setStatus(
                .failed(
                    errorMessage: String(format: String(localized: "find-and-replace.partial-update-errors", defaultValue: "%d successfully updated, %d errors occurred. Please check logs for more information.", comment: "Mixed results with some successes and errors"), updatedFilesCount, errorCount)
                )
            )
        } else {
    ...
    func replaceRange(
        ...
        guard let fileContent = try? String(contentsOf: file, encoding: .utf8) else {
            let alert = NSAlert()
            alert.messageText = String(localized: "find-and-replace.error-title", defaultValue: "Error", comment: "Error dialog title")
            alert.informativeText = String(format: String(localized: "find-and-replace.read-error", defaultValue: "An error occurred while reading file contents of: %@", comment: "Error reading file contents"), file.path)
            alert.alertStyle = .critical
            alert.addButton(withTitle: String(localized: "find-and-replace.ok-button", defaultValue: "OK", comment: "OK button title"))
            alert.runModal()
        ...
        do {
            ...
            let alert = NSAlert()
            alert.messageText = String(localized: "find-and-replace.error-title", defaultValue: "Error", comment: "Error dialog title")
            alert.informativeText = String(format: String(localized: "find-and-replace.write-error", defaultValue: "An error occurred while writing to: %@", comment: "Error writing to file"), error.localizedDescription)
            alert.alertStyle = .critical
            alert.addButton(withTitle: String(localized: "find-and-replace.ok-button", defaultValue: "OK", comment: "OK button title"))
            alert.runModal()
        }
