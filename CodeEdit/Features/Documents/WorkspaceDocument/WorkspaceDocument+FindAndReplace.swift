...
extension WorkspaceDocument.SearchState {
    ...
    func findAndReplace(query: String, replacingTerm: String) async throws {
        ...
        if updatedFilesCount == 0 && errorCount == 0 {
            ...
            await setStatus(
                .failed(
                    errorMessage: String(format: String(localized: "find-replace.all-files-failed", defaultValue: "All files failed to update. (%d) errors occurred. Check logs for more information", comment: "Error message when all files failed to update during find and replace"), errorCount)
                )
            )
        } else if updatedFilesCount > 0 && errorCount > 0 {
            ...
            await setStatus(
                .failed(
                    errorMessage: String(format: String(localized: "find-replace.partial-success", defaultValue: "%d successfully updated, %d errors occurred. Please check logs for more information.", comment: "Status message when some files updated successfully and some failed during find and replace"), updatedFilesCount, errorCount)
                )
            )
        } else {
    ...
    func replaceRange(
        ...
        guard let fileContent = try? String(contentsOf: file, encoding: .utf8) else {
            let alert = NSAlert()
            alert.messageText = String(localized: "find-replace.error-title", defaultValue: "Error", comment: "Alert title for find and replace errors")
            alert.informativeText = String(format: String(localized: "find-replace.read-error", defaultValue: "An error occurred while reading file contents of: %@", comment: "Error message when file reading fails during find and replace"), file.path)
            alert.alertStyle = .critical
            alert.addButton(withTitle: String(localized: "find-replace.ok-button", defaultValue: "OK", comment: "OK button title in find and replace error alert"))
            alert.runModal()
        ...
        do {
            ...
            let alert = NSAlert()
            alert.messageText = String(localized: "find-replace.error-title", defaultValue: "Error", comment: "Alert title for find and replace errors")
            alert.informativeText = String(format: String(localized: "find-replace.write-error", defaultValue: "An error occurred while writing to: %@", comment: "Error message when file writing fails during find and replace"), error.localizedDescription)
            alert.alertStyle = .critical
            alert.addButton(withTitle: String(localized: "find-replace.ok-button", defaultValue: "OK", comment: "OK button title in find and replace error alert"))
            alert.runModal()
        }
