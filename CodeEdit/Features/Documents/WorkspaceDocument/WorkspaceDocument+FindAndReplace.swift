...
extension WorkspaceDocument.SearchState {
    ...
    func findAndReplace(query: String, replacingTerm: String) async throws {
        ...
        if updatedFilesCount == 0 && errorCount == 0 {
            ...
            await setStatus(.failed(errorMessage: String(format: String(localized: "find-replace.no-files-matched", defaultValue: "No files in the workspace matched: %@", comment: "Error message when find and replace finds no matching files"), query)))
            ...
            await setStatus(
                .failed(
                    errorMessage: String(format: String(localized: "find-replace.partial-success", defaultValue: "%d successfully updated, %d errors occurred. Please check logs for more information.", comment: "Error message when some files update successfully but some fail during find and replace"), updatedFilesCount, errorCount)
                )
            )
        } else {
    ...
    func replaceRange(
        ...
        guard let fileContent = try? String(contentsOf: file, encoding: .utf8) else {
            let alert = NSAlert()
            alert.messageText = String(localized: "find-replace.error", defaultValue: "Error", comment: "Alert title for find and replace errors")
            alert.informativeText = String(format: String(localized: "find-replace.read-error", defaultValue: "An error occurred while reading file contents of: %@", comment: "Alert message when file cannot be read during find and replace"), file.absoluteString)
            alert.alertStyle = .critical
            alert.addButton(withTitle: String(localized: "find-replace.ok", defaultValue: "OK", comment: "Button to dismiss error alert"))
            alert.runModal()
        ...
        do {
            ...
            let alert = NSAlert()
            alert.messageText = String(localized: "find-replace.error", defaultValue: "Error", comment: "Alert title for find and replace errors")
            alert.informativeText = String(format: String(localized: "find-replace.write-error", defaultValue: "An error occurred while writing to: %@", comment: "Alert message when file cannot be written during find and replace"), error.localizedDescription)
            alert.alertStyle = .critical
            alert.addButton(withTitle: String(localized: "find-replace.ok", defaultValue: "OK", comment: "Button to dismiss error alert"))
            alert.runModal()
        }
