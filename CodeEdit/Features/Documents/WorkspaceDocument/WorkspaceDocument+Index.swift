...
extension WorkspaceDocument.SearchState {
    ...
    func addProjectToIndex() {
        ...
        let createInfo: [String: Any] = [
            ...
            "action": "create",
            "title": String(
                localized: "workspace.indexing.status.title",
                defaultValue: "Indexing | Processing files",
                comment: "Status title shown when workspace indexing is in progress"
            ),
            "message": String(
                localized: "workspace.indexing.status.message",
                defaultValue: "Creating an index to enable fast and accurate searches within your codebase.",
                comment: "Status message explaining the indexing process to users"
            ),
            "isLoading": true
        ]
        ...
        Task.detached {
            ...
            let updateInfo: [String: Any] = [
                ...
                "action": "update",
                "title": String(
                    localized: "workspace.indexing.finished.title",
                    defaultValue: "Finished indexing",
                    comment: "Status title shown when workspace indexing is complete"
                ),
                "isLoading": false
            ]
            NotificationCenter.default.post(name: .taskNotification, object: nil, userInfo: updateInfo)
