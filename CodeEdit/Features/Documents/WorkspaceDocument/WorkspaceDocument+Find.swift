...
extension WorkspaceDocument.SearchState {
    ...
    private func evaluateFile(query: String, searchResult: inout SearchResultModel) async {
        ...
        guard let fileContent = String(bytes: data, encoding: .utf8) else {
            await setStatus(.failed(errorMessage: String(localized: "workspace_document.find.decode_error", defaultValue: "Failed to decode file content.", comment: "Error message when file content cannot be decoded")))
            return
        }
        ...
        guard let regex = try? NSRegularExpression(
            ...
            await setStatus(.failed(errorMessage: String(localized: "workspace_document.find.invalid_regex_error", defaultValue: "Invalid regular expression.", comment: "Error message when regex pattern is invalid")))
            return
        }
