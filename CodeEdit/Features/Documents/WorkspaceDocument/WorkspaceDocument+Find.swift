...
extension WorkspaceDocument.SearchState {
    ...
    private func evaluateFile(query: String, searchResult: inout SearchResultModel) async {
        ...
        guard let regex = try? NSRegularExpression(
            ...
            await setStatus(.failed(errorMessage: String(localized: "find.invalid-regex", defaultValue: "Invalid regular expression.", comment: "Error message when search pattern is not a valid regular expression")))
            return
        }
