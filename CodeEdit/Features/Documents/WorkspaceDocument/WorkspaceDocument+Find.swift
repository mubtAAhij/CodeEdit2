...
extension WorkspaceDocument.SearchState {
    ...
    func search(_ query: String) async {
        ...
        guard let indexer = indexer else {
            await setStatus(.failed(errorMessage: String(localized: "find.error.no-index", defaultValue: "No index found. Try rebuilding the index.", comment: "Error message when search index is not available")))
            return
        }
    ...
    private func evaluateFile(query: String, searchResult: inout SearchResultModel) async {
        ...
        guard let regex = try? NSRegularExpression(
            ...
            await setStatus(.failed(errorMessage: String(localized: "find.error.invalid-regex", defaultValue: "Invalid regular expression.", comment: "Error message when search query is not a valid regular expression")))
            return
        }
