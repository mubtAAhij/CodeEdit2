...
struct NavigateCommands: Commands {
    ...
    var body: some Commands {
        CommandMenu(String(localized: "navigate-commands.menu", defaultValue: "Navigate", comment: "Navigate menu in menu bar")) {
            Group {
                Button(String(localized: "navigate-commands.reveal-in-project-navigator", defaultValue: "Reveal in Project Navigator", comment: "Button to reveal file in project navigator")) {
                ...
                Button(String(localized: "navigate-commands.reveal-changes-in-navigator", defaultValue: "Reveal Changes in Navigator", comment: "Button to reveal changes in navigator")) {
                ...
                Button(String(localized: "navigate-commands.open-in", defaultValue: "Open in...", comment: "Button to open file in specified location")) {
            ...
            Group {
                Button(String(localized: "navigate-commands.show-previous-tab", defaultValue: "Show Previous Tab", comment: "Button to show previous tab")) {
                    editor?.selectPreviousTab()
                }
                .keyboardShortcut("{", modifiers: [.command])
                .disabled(editor?.tabs.count ?? 0 <= 1)  // Disable if there's one or no tabs

                Button(String(localized: "navigate-commands.show-next-tab", defaultValue: "Show Next Tab", comment: "Button to show next tab")) {
