...
struct TasksCommands: Commands {
    ...
    var body: some Commands {
        CommandMenu(String(localized: "tasks-commands.menu-title", defaultValue: "Tasks", comment: "Tasks menu title")) {
            ...
            Button(String(format: String(localized: "tasks-commands.run-button", defaultValue: "Run %@", comment: "Button to run the selected task"), selectedTaskName), systemImage: "play.fill") {
            ...
            Button(String(format: String(localized: "tasks-commands.stop-button", defaultValue: "Stop %@", comment: "Button to stop the selected task"), selectedTaskName), systemImage: "stop.fill") {
            ...
            Button(String(format: String(localized: "tasks-commands.show-output-button", defaultValue: "Show %@ Output", comment: "Button to show output for the selected task"), selectedTaskName)) {
                showOutput()
            }
            ...
            Menu {
                ...
                if taskManager?.availableTasks.isEmpty ?? true {
                    Button(String(localized: "tasks-commands.create-tasks-button", defaultValue: "Create Tasks", comment: "Button to create new tasks")) {
                        openSettings()
                    }
                }
            } label: {
                Text(String(localized: "tasks-commands.choose-task-menu", defaultValue: "Choose Task...", comment: "Menu item to choose a task"))
            }
            .disabled(taskManager?.availableTasks.isEmpty == true)

            Button(String(localized: "tasks-commands.manage-tasks-button", defaultValue: "Manage Tasks...", comment: "Button to open task management settings")) {
                openSettings()
            }
