    ...
    var body: some Commands {
        CommandMenu(String(localized: "tasks-commands.menu", defaultValue: "Tasks", comment: "Tasks menu in menu bar")) {
            let selectedTaskName: String = if let selectedTask = taskManager?.selectedTask {
                String(format: String(localized: "tasks-commands.quoted-name", defaultValue: "\"%@\"", comment: "Task name with quotes"), selectedTask.name)
            } else {
                String(localized: "tasks-commands.no-task", defaultValue: "(No Selected Task)", comment: "Placeholder when no task is selected")
            }
            ...
            Button(String(format: String(localized: "tasks-commands.show-output", defaultValue: "Show %@ Output", comment: "Button to show the selected task output"), selectedTaskName)) {
                showOutput()
            }
