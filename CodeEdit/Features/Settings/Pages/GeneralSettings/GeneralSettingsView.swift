...
private extension GeneralSettingsView {
    ...
    var showEditorJumpBar: some View {
        Toggle(String(localized: "general-settings.show-jump-bar", defaultValue: "Show Jump Bar", comment: "Toggle for showing jump bar"), isOn: $settings.showEditorJumpBar)
    }

    var dimEditorsWithoutFocus: some View {
        Toggle(String(localized: "general-settings.dim-editors-without-focus", defaultValue: "Dim editors without focus", comment: "Toggle for dimming unfocused editors"), isOn: $settings.dimEditorsWithoutFocus)
    }
    ...
    var afterWindowsCloseBehaviour: some View {
        Picker(
            String(localized: "general-settings.after-last-window-closed", defaultValue: "After the last window is closed", comment: "Label for after last window closed behavior picker"),
            ...
            Text(String(localized: "general-settings.after-last-window-closed.quit", defaultValue: "Quit", comment: "Quit application option"))
    ...
    var projectNavigatorSize: some View {
        Picker(String(localized: "general-settings.project-navigator-size", defaultValue: "Project Navigator Size", comment: "Label for project navigator size picker"), selection: $settings.projectNavigatorSize) {
            Text(String(localized: "general-settings.project-navigator-size.small", defaultValue: "Small", comment: "Small navigator size option"))
                .tag(SettingsData.ProjectNavigatorSize.small)
            Text(String(localized: "general-settings.project-navigator-size.medium", defaultValue: "Medium", comment: "Medium navigator size option"))
                .tag(SettingsData.ProjectNavigatorSize.medium)
            Text(String(localized: "general-settings.project-navigator-size.large", defaultValue: "Large", comment: "Large navigator size option"))
    ...
    var shellCommand: some View {
        LabeledContent(String(localized: "general-settings.shell-command", defaultValue: "'codeedit' Shell Command", comment: "Label for shell command installation section")) {
    ...
    var updateChecker: some View {
        Section {
            LabeledContent {
                ...
                Text(String(localized: "general-settings.check-for-updates", defaultValue: "Check for updates", comment: "Label for check for updates section"))
                Text(String(format: String(localized: "general_settings.update_checker.last_checked", defaultValue: "Last checked: %@", comment: "Label showing when updates were last checked"), lastUpdatedString))
    ...
    var autoUpdateToggle: some View {
        Toggle(String(localized: "general-settings.auto-check-updates", defaultValue: "Automatically check for app updates", comment: "Toggle for automatically checking for updates"), isOn: $updater.automaticallyChecksForUpdates)
    }

    var prereleaseToggle: some View {
        Toggle(String(localized: "general-settings.include-prerelease", defaultValue: "Include pre-release versions", comment: "Toggle for including pre-release versions"), isOn: $updater.includePrereleaseVersions)
    }

    var autoSave: some View {
        Toggle(String(localized: "general-settings.auto-save", defaultValue: "Automatically save changes to disk", comment: "Toggle for automatically saving changes"), isOn: $settings.isAutoSaveOn)
    }
    ...
    private var lastUpdatedString: String {
        if let lastUpdatedDate = updater.lastUpdateCheckDate {
            ...
            return String(localized: "general_settings.update_checker.never", defaultValue: "Never", comment: "Text shown when updates have never been checked")
        }
    }
    ...
    var openInCodeEditToggle: some View {
        Toggle(String(localized: "general_settings.open_in_codeedit_toggle", defaultValue: "Show \"Open With CodeEdit\" option in Finder", comment: "Toggle to show Open With CodeEdit option in Finder"), isOn: $openInCodeEdit)
    ...
    var revealFileOnFocusChangeToggle: some View {
        Toggle(String(localized: "general_settings.reveal_file_on_focus_change", defaultValue: "Automatically reveal in project navigator", comment: "Toggle to automatically reveal files in project navigator"), isOn: $settings.revealFileOnFocusChange)
    }
