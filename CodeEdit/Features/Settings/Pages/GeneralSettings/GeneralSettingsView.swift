...
private extension GeneralSettingsView {
    ...
    var dimEditorsWithoutFocus: some View {
        Toggle(String(localized: "settings.general.dim-editors-without-focus", defaultValue: "Dim editors without focus", comment: "Toggle for dimming editors without focus"), isOn: $settings.dimEditorsWithoutFocus)
    }
    ...
    var navigatorTabBarPosition: some View {
        Picker(String(localized: "settings.general.navigator-tab-bar-position", defaultValue: "Navigator Tab Bar Position", comment: "Label for navigator tab bar position picker"), selection: $settings.navigatorTabBarPosition) {
            Text(String(localized: "settings.general.navigator-tab-bar-position.top", defaultValue: "Top", comment: "Top position option for navigator tab bar"))
                .tag(SettingsData.SidebarTabBarPosition.top)
            Text(String(localized: "settings.general.navigator-tab-bar-position.side", defaultValue: "Side", comment: "Side position option for navigator tab bar"))
    ...
    var inspectorTabBarPosition: some View {
        Picker(String(localized: "settings.general.inspector-tab-bar-position", defaultValue: "Inspector Tab Bar Position", comment: "Label for inspector tab bar position picker"), selection: $settings.inspectorTabBarPosition) {
            Text(String(localized: "settings.general.inspector-tab-bar-position.top", defaultValue: "Top", comment: "Top position option for inspector tab bar"))
                .tag(SettingsData.SidebarTabBarPosition.top)
            Text(String(localized: "settings.general.inspector-tab-bar-position.side", defaultValue: "Side", comment: "Side position option for inspector tab bar"))
                .tag(SettingsData.SidebarTabBarPosition.side)
        }
        .pickerStyle(.radioGroup)
    }
    ...
    var afterWindowsCloseBehaviour: some View {
        Picker(
            String(localized: "settings.general.after-last-window-closed", defaultValue: "After the last window is closed", comment: "Label for window close behavior picker"),
    ...
    var shellCommand: some View {
        LabeledContent(String(localized: "settings.general.shell-command", defaultValue: "'codeedit' Shell Command", comment: "Label for shell command installation section")) {
    ...
    var updateChecker: some View {
        Section {
            LabeledContent {
                Button(String(localized: "settings.general.update-checker.check-now", defaultValue: "Check Now", comment: "Button to check for updates now")) {
                    updater.checkForUpdates()
                }
            } label: {
                Text(String(localized: "settings.general.update-checker.label", defaultValue: "Check for updates", comment: "Label for update checker section"))
    ...
    var autoUpdateToggle: some View {
        Toggle(String(localized: "settings.general.auto-update", defaultValue: "Automatically check for app updates", comment: "Toggle for automatic update checking"), isOn: $updater.automaticallyChecksForUpdates)
    }
    ...
    var autoSave: some View {
        Toggle(String(localized: "settings.general.auto-save", defaultValue: "Automatically save changes to disk", comment: "Toggle for automatic saving"), isOn: $settings.isAutoSaveOn)
    }
    ...
    private var lastUpdatedString: String {
        if let lastUpdatedDate = updater.lastUpdateCheckDate {
            ...
            return String(localized: "general_settings.update_checker.never", defaultValue: "Never", comment: "Indicates updates have never been checked")
        }
    }
    ...
    var openInCodeEditToggle: some View {
        Toggle(String(localized: "settings.general.open-in-codeedit", defaultValue: "Show \"Open With CodeEdit\" option in Finder", comment: "Toggle for showing Open With CodeEdit in Finder context menu"), isOn: $openInCodeEdit)
    ...
    var revealFileOnFocusChangeToggle: some View {
        Toggle(String(localized: "settings.general.reveal-in-navigator", defaultValue: "Automatically reveal in project navigator", comment: "Toggle for automatic file reveal in project navigator"), isOn: $settings.revealFileOnFocusChange)
    }
