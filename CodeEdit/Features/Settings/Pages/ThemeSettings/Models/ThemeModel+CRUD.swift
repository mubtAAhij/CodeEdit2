...
extension ThemeModel {
    ...
    func importTheme() {
        ...
        openPanel.prompt = String(localized: "theme-model.import-prompt", defaultValue: "Import", comment: "Open panel prompt for importing themes")
        ...
        do {
            guard let oldURL = theme.fileURL else {
                throw NSError(
                    domain: "ThemeModel",
                    code: 1,
                    userInfo: [NSLocalizedDescriptionKey: String(localized: "theme-model.error-file-url-not-found", defaultValue: "Theme file URL not found", comment: "Error message when theme file URL is not found")]
                )
            }
