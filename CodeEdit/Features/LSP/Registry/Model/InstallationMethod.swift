...
enum InstallationMethod: Equatable {
    ...
    var installerDescription: String {
        guard let packageManagerType else { return String(localized: "lsp.installation_method.unknown", defaultValue: "Unknown", comment: "Unknown installation method") }
        switch packageManagerType {
        case .npm, .cargo, .golang, .pip, .sourceBuild, .github:
            return packageManagerType.userDescription
        case .nuget, .opam, .gem, .composer:
            return String(format: String(localized: "lsp.installation_method.unsupported", defaultValue: "(Unsupported) %@", comment: "Unsupported package manager type with name"), packageManagerType.userDescription)
        }
    }
