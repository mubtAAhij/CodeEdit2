...
final class PipPackageManager: PackageManagerProtocol {
    ...
    func runPipInstall(_ source: PackageSource, in packagePath: URL) -> PackageManagerInstallStep {
        ...
        return PackageManagerInstallStep(
            name: String(localized: "lsp.package_manager.install_using_pip", defaultValue: "Install Package Using pip", comment: "Package manager installation step: install using pip"),
            confirmation: .required(
                message: String(format: String(localized: "lsp.package_manager.pip_confirmation", defaultValue: "This requires the pip package %@.\nAllow CodeEdit to install this package?", comment: "Confirmation prompt for installing pip package with package name"), source.pkgName)
            )
        ) { model in
    ...
    private func updateRequirements(in packagePath: URL) -> PackageManagerInstallStep {
        let pipCommand = getPipCommand(in: packagePath)
        return PackageManagerInstallStep(
            name: String(localized: "lsp.package_manager.update_requirements", defaultValue: "Update requirements.txt", comment: "Package manager installation step: update requirements file"),
            confirmation: .none
        ) { model in
            let requirementsPath = packagePath.appending(path: "requirements.txt")
            ...
            await model.status(String(localized: "lsp.package_manager.writing_requirements", defaultValue: "Writing requirements to requirements.txt", comment: "Status message when writing requirements file"))
    ...
    private func verifyInstallation(_ source: PackageSource, in packagePath: URL) -> PackageManagerInstallStep {
        ...
        return PackageManagerInstallStep(
            ...
            guard packageFound else {
                throw PackageManagerError.installationFailed(String(format: String(localized: "lsp.package_manager.package_not_found", defaultValue: "Package %@ not found in pip list", comment: "Error when package not found in pip list with package name"), source.pkgName))
            }
        }
    }
