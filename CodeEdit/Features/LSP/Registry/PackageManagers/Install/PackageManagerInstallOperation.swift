...
final class PackageManagerInstallOperation: ObservableObject, Identifiable {
    ...
    private func runNext() async throws {
        ...
        accumulatedOutput.append(OutputItem(isStepDivider: true, contents: String(format: String(localized: "lsp.package_manager.step_progress", defaultValue: "Step %d: %@", comment: "Package manager installation step progress with step number and task name"), currentStepIdx + 1, task.name)))
